...
## Step 6: Anthropic（Claude）API キーを取得し、**使いすぎ防止の上限**を設定する

見どころ選定に Claude を使います。**ここが一番「高額請求」につながりやすい部分**なので、
キー取得と同時に「月の上限金額」を必ず設定してください。

### 6-1. API キーを発行

1. https://console.anthropic.com を開いてログイン（無ければ登録）
2. 左メニュー「API Keys」→「Create Key」
3. 名前を入力（例: `kirinuki`）→ 作成
4. 表示された `sk-ant-...` をメモ（**この画面でしか表示されません**）

### 6-2. ⚠️ 月の上限金額（スペンドリミット）を設定する【最重要】

> これを設定しないと、万が一の不正利用・バグの際に**青天井で課金**されます。
> コード側でも投稿回数や動画尺の上限を入れていますが、**最後の砦はこの Console 設定**です。

1. 左メニュー「Settings」→「Limits」（または「Billing」→「Spend limits」）を開く
2. **Monthly spend limit（月間上限）** に、自分が許容できる上限額を入力
   - 例: まずは少額（$20〜50 など）から始めて、様子を見て調整
3. 可能なら **使用量アラート（例: 上限の50%/80%）** も設定しておく
4. 保存する

### 6-3. （任意）コストの目安を知っておく

- このサービスは1本あたり「動画の文字起こし全文＋選定結果」を Claude に投げます。
- **動画が長いほどコストが上がる**ため、コード側で90分までに制限しています（`config.yaml` の `limits`）。
- 1日に処理できる本数も制限済み（全体30本／IP・メール各3本）。

- [ ] Claude API キーをメモした: `sk-ant-__________`
- [ ] **月間スペンドリミットを設定した**（最重要）
- [ ] （任意）使用量アラートを設定した

---

## Step 7: サーバーにアプリをセットアップする
...
ANTHROPIC_API_KEY=sk-ant-（Step 6 でメモした Claude の API キー）
...
FRONTEND_ORIGIN=https://your-app.vercel.app  ← Step 8 完了後に更新する
MAX_CONCURRENT_JOBS=10
```

### 7-5. Docker Compose 用のパスワードファイルを作成

```bash
# 7-4 で決めたパスワードと同じものを入力
echo "POSTGRES_PASSWORD=あなたが決めた強いパスワード" > /app/.env
```

### 7-6. nginx の設定にドメインを反映
...
## Step 9: サーバーの設定を Vercel URL で更新する

Step 8 で確定した Vercel の URL を、サーバーの設定に反映させます。
...
## Step 10: GitHub Actions で自動デプロイを設定する
